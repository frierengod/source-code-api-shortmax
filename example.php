<?php
/**
 * Contoh Script Bridge API Shortmax via StreamAPI.web.id
 * ----------------------------------------------------
...
// Fungsi bantuan buat manggil endpoint Shortmax di sistem Bridge
function callShortmaxAPI($urlTujuan, $token) {
...
// UJI COBA KODE
// -----------------------------------------------------------------
$baseUrl = "https://streamapi.web.id/p/shortmax/api/v1";

// Contoh 1: Fitur Cari Film Shortmax
echo "\n=====================[ 1. Pencarian Shortmax ]==================\n";
$kataKunci = urlencode("cinta");
$halaman = 1;
$urlCari = "{$baseUrl}/search/{$kataKunci}/{$halaman}?lang=in";

$hasilCari = callShortmaxAPI($urlCari, $API_TOKEN);
if ($hasilCari) {
...
$hasilDetail = callShortmaxAPI($urlDetail, $API_TOKEN);
if ($hasilDetail) {
...
$hasilNonton = callShortmaxAPI($urlNonton, $API_TOKEN);
if ($hasilNonton) {
...
echo "▶▶▶ https://streamapi.web.id/docs/shortmax ◀◀◀\n\n";
